...
#define FS1_MAX_SPEED ((FS2_MAX_SPEED - LOWEST_SPEED) / 2.0) + LOWEST_SPEED
#define REVERSE_MAX_SPEED ((FS2_MAX_SPEED - LOWEST_SPEED) / 3.0) + LOWEST_SPEED
#define NEUTRAL_RATE (FS2_MAX_SPEED - LOWEST_SPEED) / 3000.0
#define REVERSE_RATE (REVERSE_MAX_SPEED - LOWEST_SPEED) / 5000.0
#define FS2_RATE (FS2_MAX_SPEED - LOWEST_SPEED) / 3000.0
#define FS1_RATE (FS1_MAX_SPEED - LOWEST_SPEED) / 5000.0
...
void inputFiltering()
{
	int command_ = 0;	// Between -100 and 100
	...
	for (;;)
	{
		...
		last_commands[command_count] = command_;
		if(command_count == 0) {
			command_accum = 0;
			for(int i = 0; i < 10; i++) {
		...
		vTaskDelay(10);
        command_count = command_count == 9 ? 0 : command_count + 1;
	}
}
...
void engineControl()
{
	...
	int gear_value = 0; // 0 = neutral, 1 = FS1, 2 = FS2, 3 = Reverse
	float speed_ = LOWEST_SPEED; // Between LOWEST_SPEED and 1000.0
...
void main()
{
	...
	xTaskCreate(engineControl, "Engine Control", 512, NULL, 1, NULL);
